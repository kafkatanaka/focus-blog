/**
 * Build consumer-safe COBW registry JSON from vendor YAML (COBW repo mirror).
 * Run: node scripts/cobw/export-registry.mjs
 */
import fs from 'fs';
...
const SOURCE = path.join(ROOT, 'vendor/cobw/data/cobw-ideas.yaml');
...
function exportIdea(idea) {
  ...
  const status = mapYoutubeStatus(yt);
  return {
    cobw_id: idea.id,
    title: idea.title,
...
for (const row of ideas) {
  if (seen.has(row.cobw_id)) {
    throw new Error(`Duplicate cobw_id: ${row.cobw_id}`);
  }
...
console.log(`Wrote ${ideas.length} ideas → ${path.relative(ROOT, OUT)}`);
